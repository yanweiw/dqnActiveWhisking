...
class dqnEnv(object):
    def __init__(self, dnnpath='models/dnn_tri_hex.h5'):
        ...
        self.classify = self._build_model()
    ...
    def _build_model(self):
        ...
        '''
        model = Sequential()
        model.add(Dense(64, input_dim=95, activation='relu'))
    ...
    def explore(self):
        # renew classification network
        # self.reset()
        seq = []
        for t in range(1, max_exploration+1):
            curr_state = np.asarray(self.state).reshape(1, 95)
            action = self.act(curr_state)
            reward, terminal, loss = self.step(action)
            next_state = np.asarray(self.state).reshape(1, 95) # convert deque to nparray
            if t == max_exploration:
                terminal = True
            seq.append((curr_state, action, reward, next_state, terminal))
            if terminal:
                return seq
    ...
    def reset(self, weights):
        ...
        self.shapeT = np.random.uniform(0, 2 * np.pi)
        self.shapeS = np.random.uniform(self.min_s, self.max_s)
        self.classify.set_weights(weights)
        self.classify.compile(loss='mse', optimizer=Adam(lr=self.learning_rate))
